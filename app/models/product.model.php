...
class ProductModel{
    ...
    public function getProductsandMarcas($id){
        $query = $this->db->prepare('SELECT productos.*, marcas.id_marcas, marcas.nombre_marca FROM productos INNER JOIN marcas ON marcas.id_marcas = productos.id_marca_fk WHERE id=?');
        $query->execute([$id]);
    ...
    public function getProductsCompleto(){
        $query = $this->db->prepare('SELECT productos.*, marcas.id_marcas, marcas.nombre_marca FROM productos INNER JOIN marcas ON marcas.id_marcas = productos.id_marca_fk ORDER BY productos.id');
        $query->execute();
